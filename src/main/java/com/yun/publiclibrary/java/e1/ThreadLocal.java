...
public class ThreadLocal<T> {
    ...
    /**
     * Returns true if there is a value in the current thread's copy of this thread-local variable, even if that
     * values is null.
     *
     ...
     * need to override this method, relying soley on this  initialValue method to set the values of thread-locals.
     *
     ...
     * the sake of providing createInheritedMap factory method without needing to subclass the map class in
     * InheritableThreadLocal. This technique is preferable to the alternative of embedding instanceof tests in methods.
     */
    ...
    /**
     * ThreadLocalMap is a customized hash map suitable only for maintaining thread local values. No operations are
     * exported outside the ThreadLocal class. The class is package private to allow declaration of fields in class
     * Thread. To help deal with very large and long-lived usages, the hash table entries use WeakReferences for keys.
    ...
    public static class ThreadLocalMap {
         ...
         * so the entry can be expunged from table. Such entires are referred to as "stale entries" in the code that follows.
         */
        ...
        // The number of entires in the table.
        private int size = 0;
        ...
        /**
         * Get the entry associated with key. This method itself handles only the fast path: a direct hit of existing key.
         * It otherwise relays to getEntryAfterMiss. This is designed to maximize performance for direct hits, in part by
         * making this method readily inlinable.
         *
         ...
         *
         * @param key tye thread local object
        ...
        private void set(ThreadLocal<?> key, Object value) {
            // We don't use a fast path as with get() because it is at least
            // as common to use set() to create new entires as it is to replace
            // existing ones, in which case, a fast path would fail more often than not.
        ...
        /**
         * Replace a stale entry encountered during a set operation with an entry for the specified key. The value passed in
         * the value parameter is stored in the entry, whether an entry already exists for the specified key. As a side effect,
         * this method expunges all stale entries in the "run" containing the stale entry. (A run is a sequence of entries
        ...
        private void replaceStaleEntry(ThreadLocal<?> key, Object value, int staleSlot) {
            ...
            for (int i = nextIndex(staleSlot, len); (e = tab[i]) != null; i = nextIndex(i, len)) {
                ...
                // to remove or rehash all the other entries in run.
                if (k == key) {
                    e.value = value;
         ...
         * @return the index of the next null slot after staleSlot(all between staleSlot and this slot will have been
         *         checked for expunging).
         */
            ...
            for (i = nextIndex(staleSlot, len); (e = tab[i]) != null; i = nextIndex(i, len)) {
                ...
                if (k == null) {
                    ...
                    if (h != i) {
                        ...
                        // null because multiple entires could have been stale.
                        while (tab[h] != null)
                            h = nextIndex(h, len);
                        tab[h] = e;
                    }
         ...
         * or another stale one has been expunged. It performs a logarithmic number of scans, as a balance between no
         * scanning (fast but retains garbage) and a number of scans proportional to number of elements, that would
         * find all garbage but would cause some insertions to take O(n) time.
         *
         * @param i a position known NOT to hold a stale entry. The scan starts at the element after i
         * @param n scan control: log2(n) cells are scanned, unless a stale entry is found, in which case log2(table.length)-1
         *          additional cells are scanned. When called from insertions, this parameter is the number of elements,
         *          but when from replaceStaleEntry, it is the table length. (Note: all this could be changed to be either more
         *          or less aggressive by weighting n instead of j ust using straight log n. But this version is simple, fast,
        ...
        /**
         * Re-pack and/or re-size the table. First scan the entire table removing stale entries. If this doesn't
         * sufficiently shrink the size of the table, double the table size.
         */
        private void rehash() {
            expungeStaleEntries();
